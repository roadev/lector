...
//-------FIN - Variables de control--------
  ...
  for x := range tablas {
    ...
    if lenguaje == "Golangweb" {
      ...
      writeFile(f, "")
      writeFile(f, "const (")
      writeFile(f, "    DB_USER     = \""+db_user+"\"")
      writeFile(f, "    DB_PASSWORD = \""+db_pass+"\"")
      writeFile(f, "    DB_NAME     = \""+db_name+"\"")
      writeFile(f, ")")
      writeFile(f, "")
      writeFile(f, "func main() {")
      writeFile(f, "    dbinfo := fmt.Sprintf(\"user=%s password=%s dbname=%s sslmode=disable\", DB_USER, DB_PASSWORD, DB_NAME)")
    ...
    if lenguaje == "PHP" {
      ...
      f := createFile("BD/db_abstract_model.php")
        ...
        writeFile(f, "  private static $db_host = '"+db_host+"';")
        writeFile(f, "  private static $db_user = '"+db_user+"';")
        writeFile(f, "  private static $db_pass = '"+db_pass+"';")
        writeFile(f, "  protected $db_name = '"+db_name+"';")
        ...
        writeFile(f, "    $this->conn = new mysqli(self::$db_host, self::$db_user, self::$db_pass, $this->db_name);")
        writeFile(f, "  }")
        writeFile(f, "")
        ...
        writeFile(f, "      $this->db_name = '"+db_name+"';")
        writeFile(f, "    }")
        writeFile(f, "")
...
func getVariablesVistaListaLaravel(triples [][][]string, tablas []string, element int, tipo_resultado string) string {
    resultado := ""
    ...
    for k < cantidad  {
        ...
        } else if tipo_resultado == "table-view" {
          resultado = resultado + "<tr>"  + "\n\t\t\t"
...
// tipo_datos: 0 las llaves primarias, 1 las llaves que no son primarias
func getColumnasEspecialesPHP(triples [][][]string, element int, tipo_datos int, tipo_respuestas int) string {
    resultado := ""
    cantidad := 0
    k := 1
    suf, pref := "", ""

    if tipo_datos == 1 { //las columnas sobrantes que no son llaves primarias
        cantidad = len(triples[element][0])
        for k < cantidad  {
            if !stringInSlice(triples[element][0][k], triples[element][3]) {
